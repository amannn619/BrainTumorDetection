...
class Frames:
    ...
    def __init__(self, mainObj, MainWin, wWidth, wHeight, function, Object, xAxis=10, yAxis=10):
        ...
        global winFrame
        self.winFrame = tkinter.Frame(self.MainWindow, width=wWidth, height=wHeight, background= "gray")
        self.winFrame['borderwidth'] = 5
        self.winFrame['relief'] = 'groove'
        self.winFrame.place(x=xAxis, y=yAxis)

        self.btnClose = tkinter.Button(self.winFrame, text="Close", font="Terminal", width=8,
                                      command=lambda: self.quitProgram(self.MainWindow))
        self.btnClose.place(x=1020, y=600)
        self.btnView = tkinter.Button(self.winFrame, text="View", font="Terminal", width=8, command=lambda: self.NextWindow(self.method))
        self.btnView.place(x=750, y=550)
    ...
    def displayImage(self):
        imgTk = self.image.resize((350, 350), Image.ANTIALIAS)
