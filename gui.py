...
class Gui:
    ...
    def __init__(self):
        ...
        WindowLabel.place(x=310, y=30)
        WindowLabel.configure(background = "black", foreground = "red", font=("Terminal", 20, "bold"))

        self.val = tkinter.IntVar()
        RB1 = tkinter.Radiobutton(self.FirstFrame.getFrames(), text="Detect Tumor", font="Terminal", background = "black", foreground = "red", variable=self.val,
                                  value=1, command=self.check)
        RB1.place(x=150, y=200)
        RB2 = tkinter.Radiobutton(self.FirstFrame.getFrames(), text="View Tumor Region",  font="Terminal", background = "black", foreground = "red",
                                  variable=self.val, value=2, command=self.check)
        RB2.place(x=150, y=250)

        browseBtn = tkinter.Button(self.FirstFrame.getFrames(), text="Browse", font="Terminal", width=8, command=self.browseWindow)
        browseBtn.place(x=500, y=550)
    ...
    def check(self):
        ...
        if (self.val.get() == 1):
            ...
            if res > 0.5:
                ...
                resLabel.configure(background="black", font=("Fixedsys", 20), fg="red")
            else:
                resLabel = tkinter.Label(self.FirstFrame.getFrames(), text="No Tumor", height=1, width=20)
                resLabel.configure(background="black", font=("Fixedsys", 20), fg="green")

            resLabel.place(x=900, y=250)
            ...
            self.listOfWinFrame[0].setCallObject(self.DT)
            self.DT.removeNoise()
            self.listOfWinFrame[0].setMethod(self.DT.displayTumor)
